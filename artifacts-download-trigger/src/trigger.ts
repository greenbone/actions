...
export async function trigger(): Promise<void> {
  try {
    ...
    debug("trigger args", args);
    const workflowHandler = new WorkflowHandler(args.token, args.workflowRef, args.owner, args.repo, args.branch||args.ref);

    debug("trigger workflowHandler", workflowHandler);
