...
class ParseArgsTestCase(unittest.TestCase):
    def test_parse_args(self):
        args = parse_args([".", "--hide_scan_details", "--pr_comment_level", "DEBUG", "--filter", ".*urgs.*"])

        self.assertEqual(args.repopath, ".")
        self.assertEqual(args.filter, ".*urgs.*")
        self.assertEqual(args.hide_scan_details, True)
        self.assertEqual(args.pr_comment_level, "DEBUG")
...
class PrintMarkerTestCase(unittest.TestCase):
   def test_print_marker(self):
       detected_markers = 0
       detected_markers = print_marker([], "WARNING", False, "description", 1, 1, "fakePath", detected_markers)
...
class ScanFileTestCase(unittest.TestCase):
   def test_scan_file(self):
       ...
       detected_markers = 0
       detected_markers = scan_file([], "WARNING", False, file_path)
...
class ScanMultipleChangedFilesTestCase(unittest.TestCase):
   def test_scan_multiple_changed_files(self):
       detected_markers = scan_multiple_changed_files([], "WARNING", True, ["tests/509_pillar-security_example", "tests/12_basic_example" ])
...
class ScanSingleChangedFileTestCase(unittest.TestCase):
   def test_scan_single_changed_file(self):
       detected_markers = scan_single_changed_file([], "WARNING", True, "tests/12_basic_example")

       self.assertEqual(detected_markers, 12)

class ScanChangedFilesTestCase(unittest.TestCase):
    def test_ChangedFilesTestCase(self):
        detected_markers = scan_changed_files([], "WARNING", True, ["tests/509_pillar-security_example", "tests/12_basic_example" ])
